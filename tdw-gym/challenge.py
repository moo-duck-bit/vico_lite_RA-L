...
def _obs_has_put_in_success(state: dict) -> bool:
    """True if any agent obs reports a non-zero put_in_success object id this step."""
    for obs in state.values():
        if not isinstance(obs, dict):
            continue
        raw = obs.get("put_in_success") or ()
        if isinstance(raw, (tuple, list)):
            if any(obj_id not in (0, None) for obj_id in raw):
                return True
    return False
            ...
            try:
                ...
                while not done:
                    ...
                    if self.stop_on_first_success:
                        if _obs_has_put_in_success(state):
                            self.logger.info(
                                "Episode %s: put_in success detected (%d/%d on check_goal), stopping early for screen recording",
                                episode,
                                local_finish[0],
                                local_finish[1],
                            )
                            ...
                            break
                        if local_finish[0] > prev_goal_count and local_finish[0] >= 1:
                            self.logger.info(
                                "Episode %s: goal count %d/%d but no put_in yet — continuing so agent can reach the bed",
                                episode,
                                local_finish[0],
                                local_finish[1],
                            )
